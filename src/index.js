...
async function handleMembers(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const rows = await db.prepare(`SELECT u.id, u.username, u.role, u.is_admin, u.created_at, u.status, p.display_name, p.profile_title FROM users u LEFT JOIN user_profiles p ON p.user_id = u.id ORDER BY u.created_at DESC, u.id DESC`).all(); const ranks = await loadAccountRanks(env); if (!ranks.ok) return json(ranks, 500); const members=[]; for (const r of (rows.results||[])) { const progress=await getUserAccountProgress(db, r.id); const profile=await db.prepare('SELECT selected_rank_id FROM user_profiles WHERE user_id = ?').bind(r.id).first(); const displayedRank=getDisplayedRank(progress.accountLevel, profile?.selected_rank_id, ranks.ranks); members.push({ ...serializeUser(r), created_at:r.created_at||null, display_name:r.display_name||null, profile_title:r.profile_title||null, accountLevel:progress.accountLevel, rank: displayedRank }); } return json({ ok: true, siteName: SITE_NAME, members }); }
async function handleProfileMe(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(user.id).first(); if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, bio, location, favourite_colour, profile_title, avatar_url, banner_url, selected_rank_id FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const ranks = await loadAccountRanks(env); if (!ranks.ok) return json(ranks, 500); const progress = await getUserAccountProgress(db, row.id); const unlockedRanks = getUnlockedRanks(progress.accountLevel, ranks.ranks); const defaultRank = getDefaultRankForLevel(progress.accountLevel, ranks.ranks); const rank = getDisplayedRank(progress.accountLevel, profileRow?.selected_rank_id, ranks.ranks); return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...(profileRow||{}), ...progress, selected_rank_id: profileRow?.selected_rank_id || null, rank, defaultRank, unlockedRanks } }); }
async function handleProfileLookup(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const url = new URL(request.url); const idParam = url.searchParams.get('id'); const usernameParam = (url.searchParams.get('user') || '').trim(); let row = null; if (idParam) { const id = Number(idParam); if (!Number.isInteger(id) || id < 1) return json({ ok: false, error: 'Invalid user id' }, 400); row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(id).first(); } else if (usernameParam) { row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE username = ?').bind(usernameParam).first(); } else { return handleProfileMe(request, env); } if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, bio, location, favourite_colour, profile_title, avatar_url, banner_url, selected_rank_id FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const ranks = await loadAccountRanks(env); if (!ranks.ok) return json(ranks, 500); const progress = await getUserAccountProgress(db, row.id); const defaultRank = getDefaultRankForLevel(progress.accountLevel, ranks.ranks); const rank = getDisplayedRank(progress.accountLevel, profileRow?.selected_rank_id, ranks.ranks); const unlockedRanks = user.id === row.id ? getUnlockedRanks(progress.accountLevel, ranks.ranks) : undefined; return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...(profileRow||{}), ...progress, selected_rank_id: profileRow?.selected_rank_id || null, rank, defaultRank, ...(unlockedRanks ? { unlockedRanks } : {}) } }); }
async function handleProfileMeUpdate(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const body = await readJsonBody(request); const profile = normalizeProfileInput(body); if (profile.error) return json({ ok: false, error: profile.error }, 400); const db = getDatabase(env); await ensureSchema(db); await db.prepare("INSERT INTO user_profiles (user_id, display_name, bio, location, favourite_colour, profile_title, avatar_url, banner_url, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now')) ON CONFLICT(user_id) DO UPDATE SET display_name=excluded.display_name, bio=excluded.bio, location=excluded.location, favourite_colour=excluded.favourite_colour, profile_title=excluded.profile_title, avatar_url=excluded.avatar_url, banner_url=excluded.banner_url, updated_at=datetime('now')").bind(user.id, profile.display_name, profile.bio, profile.location, profile.favourite_colour, profile.profile_title, profile.avatar_url, profile.banner_url).run(); return handleProfileMe(request, env); } catch (error) { return json({ ok: false, error: friendlyError(error) }, 500); } }


async function handleProfileRankUpdate(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const body = await readJsonBody(request); const incoming = body?.selected_rank_id; const selected = incoming == null ? null : String(incoming).trim(); const db = getDatabase(env); await ensureSchema(db); const ranks = await loadAccountRanks(env); if (!ranks.ok) return json(ranks, 500); const progress = await getUserAccountProgress(db, user.id); let selectedRankId = null; if (selected && selected.toLowerCase() !== 'default') { const rank = getRankById(selected, ranks.ranks); if (!rank) return json({ ok: false, error: 'Unknown rank' }, 400); if (rank.level > progress.accountLevel) return json({ ok: false, error: 'You have not unlocked that rank yet' }, 400); selectedRankId = rank.id; }
...
function normalizeProfileInput(body) { const clean = (v) => String(v ?? '').trim(); const nullable = (v) => { const x = clean(v); return x || null; }; const display_name = nullable(body?.display_name); const bio = nullable(body?.bio); const location = nullable(body?.location); const favourite_colour = nullable(body?.favourite_colour); const profile_title = nullable(body?.profile_title); const avatar_url = nullable(body?.avatar_url); const banner_url = nullable(body?.banner_url); if (bio && bio.length > 500) return { error: 'bio max length is 500' }; if (profile_title && profile_title.length > 80) return { error: 'profile_title max length is 80' }; if (display_name && display_name.length > 80) return { error: 'display_name max length is 80' }; if (location && location.length > 120) return { error: 'location max length is 120' }; if (favourite_colour && favourite_colour.length > 60) return { error: 'favourite_colour max length is 60' }; if (avatar_url && avatar_url.length > 500) return { error: 'avatar_url max length is 500' }; if (banner_url && banner_url.length > 500) return { error: 'banner_url max length is 500' }; return { display_name, bio, location, favourite_colour, profile_title, avatar_url, banner_url }; }
