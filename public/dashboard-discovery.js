(() => {
  ...
  function calendarTile(feature,editing){
    ...
    if(!editing){const controls=document.createElement('div');controls.className='dashboard-calendar-controls';[['−1','Previous month'],['0','Current month'],['1','Next month']].forEach(([delta,text])=>{const button=document.createElement('button');button.type='button';button.textContent=delta==='-1'?'‹':delta==='1'?'›':'Today';button.title=text;button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();moduleState.monthOffset=delta==='0'?0:moduleState.monthOffset+Number(delta);renderDashboardGrid();});controls.append(button);});heading.append(controls);}
  ...
  function ensurePicker(){
    ...
    dialog.querySelector('[data-close-tile-picker]')?.addEventListener('click',()=>dialog.close());dialog.addEventListener('click',event=>{if(event.target===dialog)dialog.close();const button=event.target.closest('#dashboard-catalogue button');if(button&&button.textContent==='Place tile')setTimeout(()=>dialog.close(),0);});return dialog;
  }
