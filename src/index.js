...
async function createSchemaTables(db) { await db.prepare("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'admin', is_admin INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')))" ).run(); try { await db.prepare("ALTER TABLE users ADD COLUMN status TEXT NOT NULL DEFAULT 'active'").run(); } catch {}
  ...
  await db.prepare("CREATE TABLE IF NOT EXISTS showcase_catalog (id INTEGER PRIMARY KEY AUTOINCREMENT, item_key TEXT NOT NULL UNIQUE, item_type TEXT NOT NULL, name TEXT NOT NULL, description TEXT, icon_url TEXT, source TEXT, rarity TEXT NOT NULL DEFAULT 'common', is_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))").run();
...
async function handleAdminShowcaseCatalog(request, env){const auth=await requireAdmin(request, env); if(auth) return auth; const db=getDatabase(env); await ensureSchema(db); const rows=await db.prepare("SELECT * FROM showcase_catalog ORDER BY id DESC").all(); return json({ok:true,items:rows.results||[]});}
async function handleAdminShowcaseCatalogCreate(request, env){const auth=await requireAdmin(request, env); if(auth) return auth; const b=await readJsonBody(request); const db=getDatabase(env); await ensureSchema(db); await db.prepare("INSERT INTO showcase_catalog (item_key,item_type,name,description,icon_url,source,rarity,is_active,updated_at) VALUES (?,?,?,?,?,?,?,?,datetime('now'))").bind(String(b.item_key||'').trim().toLowerCase(),String(b.item_type||'other').trim(),String(b.name||'').trim(),String(b.description||'').trim()||null,String(b.icon_url||'').trim()||null,String(b.source||'admin').trim()||null,String(b.rarity||'common').trim(),Number(b.is_active)===0?0:1).run(); return json({ok:true});}
async function handleAdminShowcaseCatalogUpdate(request, env, path){const auth=await requireAdmin(request, env); if(auth) return auth; const id=Number(path.split('/').pop()); const b=await readJsonBody(request); const db=getDatabase(env); await ensureSchema(db); await db.prepare("UPDATE showcase_catalog SET item_key=?,item_type=?,name=?,description=?,icon_url=?,source=?,rarity=?,is_active=?,updated_at=datetime('now') WHERE id=?").bind(String(b.item_key||'').trim().toLowerCase(),String(b.item_type||'other').trim(),String(b.name||'').trim(),String(b.description||'').trim()||null,String(b.icon_url||'').trim()||null,String(b.source||'admin').trim()||null,String(b.rarity||'common').trim(),Number(b.is_active)===0?0:1,id).run(); return json({ok:true});}
async function handleAdminShowcaseCatalogToggle(request, env, path){const auth=await requireAdmin(request, env); if(auth) return auth; const id=Number(path.split('/')[5]); const db=getDatabase(env); await ensureSchema(db); await db.prepare("UPDATE showcase_catalog SET is_active = CASE WHEN is_active=1 THEN 0 ELSE 1 END, updated_at=datetime('now') WHERE id=?").bind(id).run(); return json({ok:true});}
async function handleAdminGrantCatalogUnlock(request, env, path){const auth=await requireAdmin(request, env); if(auth) return auth; const userId=Number(path.split('/')[4]); const b=await readJsonBody(request); const db=getDatabase(env); await ensureSchema(db); await db.prepare("INSERT OR IGNORE INTO user_showcase_unlocks (user_id,catalog_item_id,unlock_source) VALUES (?,?,?)").bind(userId,Number(b.catalog_item_id),String(b.unlock_source||'admin')).run(); return json({ok:true});}
