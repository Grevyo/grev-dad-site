...
async function handleProfileCardMeUpdate(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const body = await readJsonBody(request); const card_layout = ['compact', 'standard', 'showcase'].includes(String(body?.card_layout || 'standard')) ? String(body?.card_layout || 'standard') : null; if (!card_layout) return json({ ok: false, error: 'Invalid card_layout' }, 400); const card_background_url = normalizeImageUrl(body?.card_background_url); if (card_background_url === null) return json({ ok: false, error: 'Invalid card image URL' }, 400); const cardTileSettings = normalizeCardTileSettings(body?.card_tile_settings ?? body?.cardTileSettings); const cardTileSettingsJson = JSON.stringify(cardTileSettings); if (cardTileSettingsJson.length > 8000) return json({ ok: false, error: 'cardTileSettings too large' }, 400); const db = getDatabase(env); await ensureSchema(db); await db.prepare('INSERT INTO user_profiles (user_id) VALUES (?) ON CONFLICT(user_id) DO NOTHING').bind(user.id).run(); await db.prepare(`UPDATE user_profiles SET card_background_url=?, card_background_colour=?, card_accent_colour=?, card_text_colour=?, card_body_text_colour=?, card_border_colour=?, card_layout=?, card_show_avatar=?, card_show_display_name=?, card_show_username=?, card_show_user_id=?, card_show_role=?, card_show_level=?, card_show_rank=?, card_show_xp=?, card_show_status=?, card_show_steam=?, card_show_leetify=?, card_tile_settings_json=?, updated_at=datetime('now') WHERE user_id=?`).bind(card_background_url || '', normalizeHexColour(body?.card_background_colour), normalizeHexColour(body?.card_accent_colour), normalizeHexColour(body?.card_text_colour), normalizeHexColour(body?.card_body_text_colour), normalizeHexColour(body?.card_border_colour), card_layout, normalizeBool01(body?.card_show_avatar, 1), normalizeBool01(body?.card_show_display_name, 1), normalizeBool01(body?.card_show_username, 1), normalizeBool01(body?.card_show_user_id, 0), normalizeBool01(body?.card_show_role, 1), normalizeBool01(body?.card_show_level, 1), normalizeBool01(body?.card_show_rank, 1), normalizeBool01(body?.card_show_xp, 1), normalizeBool01(body?.card_show_status, 0), normalizeBool01(body?.card_show_steam, 0), normalizeBool01(body?.card_show_leetify, 0), cardTileSettingsJson, user.id).run(); return handleProfileMe(request, env); } catch (error) { return json({ ok: false, error: friendlyError(error) }, 500); } }
