...
const PUBLIC_HTML_PATHS = new Set(['/unregistered.html', '/login.html', '/register.html', '/GrevNetWatch', '/GrevNetWatch/', '/GrevNetWatch/index.html']);
...
export default { async fetch(request, env) { try { const url = new URL(request.url);
  if (url.pathname === '/GrevNetWatch') return Response.redirect(new URL('/GrevNetWatch/', request.url), 302);
...
function safeCs2Status(row, counts={}) {
  ...
  if (imported > 0) return 'synced_matches_imported';
  const stored=String(row.sync_status||'').trim();
  if (stored==='synced') return 'synced_matches_imported';
  if (stored==='importer_not_implemented') return 'share_codes_found_parser_pending';
  if (stored==='sync_failed' || stored==='cs2_no_key_fetch_unavailable') return stored;
  if (discovered > 0 && imported === 0) return 'share_codes_found_parser_pending';
  if (stored) return stored;
  if (row.last_checked_at) return 'checked_no_newer_match';
  return 'connected_not_synced';
}
function redactCs2SensitiveText(value){
  return String(value||'')
    .replace(/CSGO-[A-Za-z0-9_-]+/g,'[redacted share code]')
    .replace(/((?:steamidkey|knowncode|authcode|cs2_auth_code|latest_known_share_code|sharecode|nextcode)=)[^&\s"'<>]+/gi,'$1[redacted]')
    .replace(/("(?:steamidkey|knowncode|authCode|cs2_auth_code|latest_known_share_code|sharecode|nextcode)"\s*:\s*)"[^"]*"/gi,'$1"[redacted]"');
}
function safeSyncErrorMessage(value){ const text=redactCs2SensitiveText(value).trim(); if(!text)return ''; return text.slice(0,360); }
...
function cs2SyncError(code,error,status=400,detail={}){
  const body={ok:false,code,error:safeSyncErrorMessage(error)||'CS2 sync failed.'};
  const safeDetail=safeSyncErrorMessage(detail.safeDetail);
  const responseSnippet=safeSyncErrorMessage(detail.responseSnippet);
  if(safeDetail) body.safeDetail=safeDetail;
  if(Number.isInteger(detail.httpStatus)) body.httpStatus=detail.httpStatus;
  if(typeof detail.responseWasJson==='boolean') body.responseWasJson=detail.responseWasJson;
  if(responseSnippet) body.responseSnippet=responseSnippet;
  if(detail.detectedErrorType) body.detectedErrorType=String(detail.detectedErrorType).replace(/[^a-z0-9_-]/gi,'').slice(0,80);
...
const CS2_VALVE_DENIED_RE=/access is denied|forbidden|unauthorized|not authorized|invalid authentication|auth(?:entication)? failed|invalid steamidkey|invalid knowncode/i;
const CS2_VALVE_RATE_LIMIT_RE=/rate limit|too many requests|try again later/i;
function safeCs2ValveSnippet(text){ const clean=redactCs2SensitiveText(text).replace(/\s+/g,' ').trim(); return clean.slice(0,180); }
function cs2ValveFailure({code,error,safeDetail,httpStatus,responseWasJson,responseSnippet,detectedErrorType}){ return { ok:false, code, error, safeDetail, httpStatus, responseWasJson, responseSnippet, detectedErrorType }; }
function classifyCs2ValveFailure(resp,text,responseWasJson){
  const httpStatus=Number(resp?.status)||0;
  const responseSnippet=safeCs2ValveSnippet(text);
  const mentionsDenied=CS2_VALVE_DENIED_RE.test(text);
  const mentionsRateLimit=CS2_VALVE_RATE_LIMIT_RE.test(text);
  const detailPrefix=httpStatus?`HTTP ${httpStatus} from Valve.`:'Valve returned a response without an HTTP status.';
  if(httpStatus===401||httpStatus===403||mentionsDenied){
    const code=httpStatus?`CS2_VALVE_HTTP_${httpStatus}`:'CS2_VALVE_REJECTED';
    return cs2ValveFailure({code,error:'Valve rejected the CS2 match-history request.',safeDetail:`${detailPrefix} ${mentionsDenied?'Response mentions access denied or invalid saved match-history details.':'Valve did not allow the saved match-history details.'}`,httpStatus,responseWasJson,responseSnippet,detectedErrorType:'access_denied'});
  }
  if(httpStatus===429||mentionsRateLimit) return cs2ValveFailure({code:'CS2_VALVE_RATE_LIMITED',error:'Valve rate-limited the CS2 match-history request.',safeDetail:`${detailPrefix} Try syncing again later.`,httpStatus,responseWasJson,responseSnippet,detectedErrorType:'rate_limited'});
  if(httpStatus>=500) return cs2ValveFailure({code:`CS2_VALVE_HTTP_${httpStatus}`,error:'Valve match-history service returned an error.',safeDetail:`${detailPrefix} Valve appears to be temporarily unavailable.`,httpStatus,responseWasJson,responseSnippet,detectedErrorType:'valve_server_error'});
  return cs2ValveFailure({code:httpStatus?`CS2_VALVE_HTTP_${httpStatus}`:'CS2_VALVE_BAD_RESPONSE',error:'Valve match-history request failed.',safeDetail:detailPrefix,httpStatus,responseWasJson,responseSnippet,detectedErrorType:'unexpected_http_response'});
}
async function fetchNextCs2ShareCode({steamId64,authCode,knownCode}){
  const params=new URLSearchParams({ steamid:steamId64, steamidkey:authCode, knowncode:knownCode });
  const url=`https://api.steampowered.com/ICSGOPlayers_730/GetNextMatchSharingCode/v1/?${params.toString()}`;
  let resp;
  try { resp=await fetch(url,{headers:{'Accept':'application/json,text/plain,*/*'}}); }
  catch { return cs2ValveFailure({code:'CS2_VALVE_NETWORK_ERROR',error:'Valve match-history request could not be reached.',safeDetail:'The request failed before Valve returned an HTTP status.',detectedErrorType:'network_error'}); }
  const text=await resp.text().catch(()=>'');
  let payload=null;
  let responseWasJson=false;
  if(/^\s*[\[{]/.test(text)){ try{ payload=JSON.parse(text); responseWasJson=true; }catch{} }
  if(!resp.ok) return classifyCs2ValveFailure(resp,text,responseWasJson);
  if(CS2_VALVE_DENIED_RE.test(text)||CS2_VALVE_RATE_LIMIT_RE.test(text)) return classifyCs2ValveFailure(resp,text,responseWasJson);
  const nextCode=textOrNull(payload?.result?.nextcode||payload?.response?.nextcode||payload?.nextcode||payload?.result?.next_code);
  return { ok:true, nextCode, payload };
}
async function discoverCs2ShareCodes({steamId64,authCode,latestShareCode}){ const discovered=[]; const seen=new Set([String(latestShareCode||'')]); let knownCode=latestShareCode; for(let i=0;i<10;i++){ const next=await fetchNextCs2ShareCode({steamId64,authCode,knownCode}); if(!next.ok) return { ok:false, code:next.code, error:next.error, safeDetail:next.safeDetail, httpStatus:next.httpStatus, responseWasJson:next.responseWasJson, responseSnippet:next.responseSnippet, detectedErrorType:next.detectedErrorType, discovered }; if(!next.nextCode || seen.has(next.nextCode)) return { ok:true, discovered }; discovered.push(next.nextCode); seen.add(next.nextCode); knownCode=next.nextCode; } return { ok:true, discovered, truncated:true }; }
...
async function importCs2MatchesFromShareCodes(db,userId,discoveredCodes){ const codes=Array.isArray(discoveredCodes)?discoveredCodes.filter((code)=>String(code||'').trim()):[]; const latestShareCode=codes[codes.length-1]||''; for(const code of codes){ const safeRef=await safeHashRef(`${userId}:${code}`); await db.prepare("UPDATE cs2_match_share_codes SET import_status='parser_pending', import_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND safe_share_ref=?").bind(userId,safeRef).run().catch(async()=>{ await db.prepare("UPDATE cs2_match_share_codes SET import_status='parser_pending', import_error='' WHERE user_id=? AND safe_share_ref=?").bind(userId,safeRef).run(); }); } if(latestShareCode){ await db.prepare("UPDATE cs2_match_connections SET latest_known_share_code=?, sync_status='share_codes_found_parser_pending', last_sync_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(latestShareCode,userId).run(); } else { await db.prepare("UPDATE cs2_match_connections SET sync_status='share_codes_found_parser_pending', last_sync_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(userId).run(); } return { ok:true, status:'parser_pending', syncStatus:'share_codes_found_parser_pending', statsUpdated:false, importedMatchesCount:0, modesUpdated:[], message:`CS2 sync discovered ${codes.length} new match share code${codes.length===1?'':'s'}. Full match stats are not available until the importer is implemented.` }; }
// TODO: Wire this only after a real CS2 share-code parser/importer returns parsed match objects.
// Do not call it with placeholder matches; cs2_mode_public_stats must reflect real parsed match data only.
async function writeCs2MatchStats(db,userId,matches,latestShareCode){ const safeMatches=[]; for(const match of matches){ const mode=isSafeCs2Mode(match.mode)?match.mode:'unknown'; const safeRef=await safeHashRef(match.rawRef||`${userId}:${mode}:${match.match_date||''}:${match.map||''}:${match.team_score??''}:${match.enemy_score??''}`); await db.prepare('INSERT INTO cs2_imported_matches (user_id, mode, safe_share_ref, map, match_date, team_score, enemy_score, result, kills, deaths, assists, kd, rank_before, rank_after) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(userId, mode, safeRef, match.map, match.match_date, match.team_score, match.enemy_score, match.result, match.kills, match.deaths, match.assists, match.kd, match.rank_at_time, match.rank_after||null).run(); if(isSafeCs2Mode(mode)) safeMatches.push({...match,mode}); }
  ...
  if(modesUpdated.length){ if(latestShareCode) await db.prepare("UPDATE cs2_match_connections SET latest_known_share_code=?, sync_status='synced_matches_imported', last_successful_sync_at=CURRENT_TIMESTAMP, last_sync_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(latestShareCode,userId).run(); else await db.prepare("UPDATE cs2_match_connections SET sync_status='synced_matches_imported', last_successful_sync_at=CURRENT_TIMESTAMP, last_sync_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(userId).run(); } else if(latestShareCode) await db.prepare('UPDATE cs2_match_connections SET latest_known_share_code=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?').bind(latestShareCode,userId).run(); return modesUpdated; }
async function handleCs2Sync(request, env) { try { const ctx=await getOwnerConnectionUser(request, env); if(ctx.error) return ctx.error; const userId=String(ctx.user.id); const row=await ctx.db.prepare('SELECT user_id, steam_id64, cs2_auth_code, latest_known_share_code FROM cs2_match_connections WHERE user_id=? AND is_enabled=1').bind(userId).first(); if(!row) return cs2SyncError('CS2_CONNECTION_MISSING','CS2 Match History is not connected.'); if(!row.steam_id64 || !row.cs2_auth_code || !row.latest_known_share_code) return cs2SyncError('CS2_CREDENTIALS_INCOMPLETE','CS2 Match History needs SteamID64, authentication code, and latest match token.'); await ctx.db.prepare("UPDATE cs2_match_connections SET last_checked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(userId).run(); const result=await discoverCs2ShareCodes({steamId64:row.steam_id64,authCode:row.cs2_auth_code,latestShareCode:row.latest_known_share_code}); if(!result.ok){ const status='sync_failed'; const storedError=safeSyncErrorMessage([result.error,result.safeDetail].filter(Boolean).join(' '))||'CS2 sync failed.'; await ctx.db.prepare("UPDATE cs2_match_connections SET sync_status=?, last_sync_error=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(status,storedError,userId).run(); return cs2SyncError(result.code||'CS2_VALVE_SYNC_FAILED',result.error||'CS2 sync failed.',502,result); } const codes=Array.isArray(result.discovered)?result.discovered:[]; if(!codes.length){ const nextStatus='checked_no_newer_match'; await ctx.db.prepare("UPDATE cs2_match_connections SET sync_status=?, last_sync_error='', updated_at=CURRENT_TIMESTAMP WHERE user_id=?").bind(nextStatus,userId).run(); const counts=await cs2ConnectionCounts(ctx.db,userId); return json({ok:true,checked:true,statsUpdated:false,discoveredShareCodesCount:0,storedShareCodesCount:counts.discoveredShareCodesCount,importedMatchesCount:counts.importedMatchesCount,syncStatus:nextStatus,message:'CS2 connection checked. Valve did not report a newer match share code.'},200,{'Cache-Control':'no-store'}); } const inserted=await saveDiscoveredCs2ShareCodes(ctx.db,userId,codes); const importer=await importCs2MatchesFromShareCodes(ctx.db,userId,codes); const counts=await cs2ConnectionCounts(ctx.db,userId); if(importer?.statsUpdated){ const imported=Number(importer.importedMatchesCount)||Number(counts.importedMatchesCount)||0; return json({ok:true,checked:true,statsUpdated:true,discoveredShareCodesCount:inserted,storedShareCodesCount:counts.discoveredShareCodesCount,importedMatchesCount:imported,syncStatus:'synced_matches_imported',message:`CS2 sync imported ${imported} match${imported===1?'':'es'}.`},200,{'Cache-Control':'no-store'}); } const syncStatus=importer?.syncStatus||'share_codes_found_parser_pending'; return json({ok:true,checked:true,statsUpdated:false,discoveredShareCodesCount:inserted,storedShareCodesCount:counts.discoveredShareCodesCount,importedMatchesCount:counts.importedMatchesCount,syncStatus,message:importer?.message||`CS2 sync discovered ${codes.length} new match share code${codes.length===1?'':'s'}. Full match stats are not available until the importer is implemented.`},200,{'Cache-Control':'no-store'}); } catch(error){ return json({ok:false,error:friendlyError(error)},500); } }
...
function isPublicPath(pathname) { return PUBLIC_HTML_PATHS.has(pathname) || PUBLIC_API_PATHS.has(pathname) || pathname === '/favicon.ico' || pathname.startsWith('/styles') || pathname.startsWith('/scripts/') || pathname.startsWith('/data/') || pathname.startsWith('/downloads/'); }
