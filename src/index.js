...
function clearSessionCookie() {
  return "session_v3=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT";
}
  ...
  try {
    ...
    if (Date.now() - lastSync > sixHours) {
      const res = await fetch(
        `https://api.pricempire.com/v1/items/prices?api_key=${SKIN_API_KEY}&sources=steam`,
        {
          headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) grev-dad-site/1.0"
          }
        }
      );
        ...
        for (const [fullName, details] of Object.entries(data)) {
          ...
          if (price > 0) {
            await env.CASES_DB.prepare(
              "UPDATE item_definitions SET base_price = ? WHERE (weapon_type || ' | ' || skin_name) = ?"
            )
              .bind(price, fullName)
              .run();
          }
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/api/register" && request.method === "POST") {
        ...
        if (existing) {
          return redirect(request, "/register.html?msg=Username%20already%20taken.");
        }
        ...
        await env.DB.prepare(`
          ...
          VALUES (?, ?, 0, 0, ?, ?)
        `)
          .bind(username, finalHash, new Date().toISOString(), null)
          .run();
      ...
      if (path === "/api/login" && request.method === "POST") {
        ...
        if (!user) {
          return redirect(request, "/login.html?msg=Invalid%20Credentials");
        }

        const parsed = parseStoredHash(user.password_hash);
        if (!parsed) {
          return redirect(request, "/login.html?msg=Invalid%20Password%20Format");
        }
        ...
        if (!user.approved) {
          return redirect(request, "/login.html?msg=Not%20Approved");
        }
      ...
      if (path === "/api/members") {
        ...
        return json({
          members: (members.results || []).map(m => ({
            ...m,
            status: getStatus(m.last_seen_at)
          }))
        });
      }

      // --- GLOBAL CHAT: LIST MESSAGES ---
      if (path === "/api/chat/global/messages" && request.method === "GET") {
        const user = await requireApprovedUser(request, env);
        if (!user) return json({ error: "Unauthorized" }, 401);
      ...
      if (path === "/api/chat/global/send" && request.method === "POST") {
        ...
        const authorGroup = user.is_admin ? "Admin" : "Member";
      ...
      if (path === "/api/cases/list") {
        ...
        const { results } = await env.CASES_DB.prepare(`
      ...
      if (path === "/api/cases/open" && request.method === "POST") {
        ...
        const { results: skins } = await env.CASES_DB.prepare(`
          SELECT *
          FROM item_definitions
          WHERE case_name = ?
        `)
        ...
        if (!skins || skins.length === 0) {
          return json({ error: "Case not found or empty" }, 404);
        }
        ...
        await env.DB.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `)
          .bind(user.id, fullName, winner.rarity, qual.price, new Date().toISOString())
          .run();
      ...
      if (path === "/api/admin/mega-seed") {
        ...
        try {
          ...
          if (skinEntries.length === 0) {
            return json({ error: "API returned no items" }, 500);
          }
          ...
          for (const [fullName, details] of skinEntries) {
            ...
            if (price > 0) {
              ...
              if (price > 200 || weaponType.includes("Knife") || weaponType.includes("Gloves")) {
                rarity = "rare_special";
                weight = 1;
              } else if (price > 100) {
                rarity = "covert";
              ...
              await env.CASES_DB.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?)
              `)
                .bind("Global Collection", weaponType, skinName, rarity, price, weight)
                .run();
          ...
          return json({
            success: true,
            message: `Seeded ${addedCount} skins into 'Global Collection'`
          });
        } catch (e) {
      ...
      if (path === "/api/admin/create-case" && request.method === "POST") {
        ...
        for (const s of skins) {
          ...
          await env.CASES_DB.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `)
            .bind(name, s.weapon, s.skin, s.rarity, s.price, weight)
            .run();
        }

        if (env.DB) {
          await env.DB.prepare(`
            INSERT OR REPLACE INTO case_configs (case_name, price)
            VALUES (?, ?)
          `)
            .bind(name, parseFloat(price))
            .run();
        }
      ...
      if (path === "/api/user/inventory") {
        ...
        return json({
          user: {
            ...userBase,
            status: getStatus(userBase.last_seen_at)
          },
          inventory: items
        });
      }
