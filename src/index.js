...
async function getUserProfileBundle(request, env, userId, { includeUnlockedRanks = false } = {}) {
  ...
  if (!row) return null;
  const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, steam_url, leetify_url, leetify_profile_url, leetify_steam_id, refrag_url, refrag_profile_url, manual_refrag_rating, manual_refrag_counter_strafe_pct, manual_refrag_reaction_time, manual_refrag_ttd, manual_refrag_crosshair_drift, manual_refrag_last_updated_at, avatar_url, banner_url, banner_display_size, profile_background_url, profile_background_size, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, status_message, profile_quote, favourite_game, profile_visibility, show_member_card, show_profile_showcase, show_profile_xp, show_profile_user_id, show_joined_date, show_header_avatar, show_header_display_name, show_header_username, show_header_user_id, show_header_level, show_header_rank, show_header_xp_bar, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_body_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, card_show_refrag, card_show_leetify, card_show_leetify_rank, card_show_leetify_rating, card_show_leetify_steam_id, card_show_leetify_avatar, card_show_leetify_name, card_show_leetify_aim, card_show_leetify_positioning, card_show_leetify_utility, card_show_leetify_clutch, card_show_leetify_opening, card_show_leetify_recent_matches, card_show_leetify_premier, card_show_leetify_map_ranks, card_show_leetify_updated, card_grid_columns, card_tile_settings_json, profile_page_background_url, profile_page_background_colour, profile_page_background_size, profile_page_overlay_strength, profile_page_tile_layout_json, profile_page_widget_settings_json, dashboard_background_colour, dashboard_background_url, dashboard_background_size, dashboard_background_overlay_strength, profile_footer_url, profile_footer_display_size, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first();
  const safeProfile = { display_name: row.username, bio: '', profile_title: '', avatar_url: '', banner_url: '', banner_display_size: 'wide', profile_background_url: '', profile_background_size: 'cover', leetify_profile_url: '', leetify_steam_id: '', refrag_url: '', refrag_profile_url: '', manual_refrag_rating: '', manual_refrag_counter_strafe_pct: '', manual_refrag_reaction_time: '', manual_refrag_ttd: '', manual_refrag_crosshair_drift: '', manual_refrag_last_updated_at: '', profile_layout: 'standard', show_level: 1, show_rank: 1, show_badges: 1, show_last_active: 0, card_background_url: '', card_background_colour: '', card_accent_colour: '', card_text_colour: '', card_body_text_colour: '', card_border_colour: '', card_layout: 'standard', card_show_avatar: 1, card_show_display_name: 1, card_show_username: 1, card_show_user_id: 0, card_show_role: 1, card_show_level: 1, card_show_rank: 1, card_show_xp: 1, card_show_status: 0, card_show_steam: 0, card_show_refrag: 0, card_show_leetify: 0, card_show_leetify_rank: 0, card_show_leetify_rating: 0, card_show_leetify_steam_id: 0, card_show_leetify_avatar: 0, card_show_leetify_name: 0, card_show_leetify_aim: 0, card_show_leetify_positioning: 0, card_show_leetify_utility: 0, card_show_leetify_clutch: 0, card_show_leetify_opening: 0, card_show_leetify_recent_matches: 0, card_show_leetify_premier: 0, card_show_leetify_map_ranks: 0, card_show_leetify_updated: 0, card_grid_columns: 4, card_tile_settings_json: '', profile_page_background_url: '', profile_page_background_colour: '', profile_page_background_size: 'cover', profile_page_overlay_strength: 20, dashboard_background_colour: '', dashboard_background_url: '', dashboard_background_size: 'cover', dashboard_background_overlay_strength: 0, profile_page_tile_layout_json: '', profile_page_widget_settings_json: '', profile_footer_url:'', profile_footer_display_size:'wide', ...(profileRow || {}) };
...
function normalizeDashboardBackgroundPayload(body = {}) {
  ...
  if (colour && !/^#[0-9a-fA-F]{6}$/.test(colour)) return { error: 'dashboard_background_colour must be blank or #RRGGBB' };
  const rawUrl = String(body.dashboard_background_url || '').trim();
  if (/^(javascript|data|file|vbscript):/i.test(rawUrl)) return { error: 'Unsupported dashboard background URL scheme' };
  let safeUrl = '';
  if (rawUrl) {
    try { const parsed = new URL(rawUrl); if (!['http:', 'https:'].includes(parsed.protocol)) return { error: 'dashboard_background_url must be http or https' }; safeUrl = parsed.href; }
    catch { return { error: 'dashboard_background_url must be a valid URL' }; }
  }
  const size = String(body.dashboard_background_size || 'cover').trim();
  if (!DASHBOARD_BACKGROUND_SIZES.has(size)) return { error: 'dashboard_background_size is invalid' };
  const overlay = Number.parseInt(body.dashboard_background_overlay_strength ?? 0, 10);
  if (!Number.isInteger(overlay) || overlay < 0 || overlay > 80) return { error: 'dashboard_background_overlay_strength must be 0-80' };
  return { dashboard_background_colour: colour, dashboard_background_url: safeUrl, dashboard_background_size: size, dashboard_background_overlay_strength: overlay };
}
async function handleDashboardBackgroundUpdate(request, env) {
  try {
    ...
    const normalized = normalizeDashboardBackgroundPayload(await readJsonBody(request));
    ...
    return json({ ok:true, background: normalized });
