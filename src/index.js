...
async function handleRegister(request, env) {
  ...
  if (!username || !email || password.length < 8) {
    return json({ success: false, error: "username, email, and password (>=8 chars) are required" }, 400);
  }
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO users (username, email, password_hash, role, is_admin, balance_cents, created_at)
       VALUES (?, ?, ?, 'user', 0, 0, ?)`
    ).bind(username, email, passwordHash, createdAt).run();
  } catch (error) {
    if (String(error.message).includes("UNIQUE")) {
      return json({ success: false, error: "Username or email already exists" }, 409);
    }
    throw error;
  }

  return json({ success: true, message: "Registered successfully" }, 201);
}
...
async function handleLogin(request, env) {
  ...
  if (!identity || !password) return json({ success: false, error: "Missing credentials" }, 400);

  const user = await env.DB.prepare(
    `SELECT id, username, email, password_hash, role, is_admin, balance_cents FROM users WHERE username = ? OR email = ? LIMIT 1`
  ...
  await env.DB.prepare("INSERT INTO sessions (token, user_id, expires_at, created_at) VALUES (?, ?, ?, ?)")
    .bind(hashToken(token), user.id, expiresAt, new Date().toISOString()).run();

  const response = json({ success: true, user: sanitizeUser(user) }, 200);
...
async function handleLogout(request, env) {
  ...
  if (token) {
    await env.DB.prepare("DELETE FROM sessions WHERE token = ?").bind(hashToken(token)).run();
  }
  const response = json({ success: true, message: "Logged out" }, 200);
...
async function handleMe(request, env) {
  ...
  return json({ success: true, authenticated: Boolean(user), user: user ? sanitizeUser(user) : null }, 200);
}

async function handleBalance(request, env) {
  const user = await currentUser(request, env);
  if (!user) return json({ success: false, error: "Unauthorized" }, 401);
  return json({ success: true, balanceCents: Number(user.balance_cents || 0) }, 200);
}

async function handleAdminUsers(request, env) {
  const user = await requireAdmin(request, env);
  if (!user) return json({ success: false, error: "Admin access required" }, 403);

  const users = await env.DB.prepare(
    "SELECT id, username, email, role, is_admin, balance_cents, created_at FROM users ORDER BY id ASC"
  ).all();

  return json({ success: true, currentUser: sanitizeUser(user), users: users.results || [] }, 200);
}
...
async function handleMakeAdmin(request, env) {
  ...
  const body = await safeJson(request);
  const identity = String(body.username || body.email || "").trim().toLowerCase();
  if (!identity) return json({ success: false, error: "username or email is required" }, 400);

  const result = await env.DB.prepare(
    "UPDATE users SET is_admin = 1, role = 'admin' WHERE username = ? OR email = ?"
  ).bind(identity, identity).run();

  return json({ success: true, updated: result.meta?.changes || 0 }, 200);
}

async function handleAdminSetupPromote(request, env) {
  const body = await safeJson(request);
  const secret = String(body.secret || "");
  const identity = String(body.username || body.email || "grev").trim().toLowerCase();

  if (!env.ADMIN_SETUP_SECRET || secret !== env.ADMIN_SETUP_SECRET) {
...
async function currentUser(request, env) {
  ...
  return env.DB.prepare(
    `SELECT u.id, u.username, u.email, u.role, u.is_admin, u.balance_cents
     FROM sessions s JOIN users u ON u.id = s.user_id
     WHERE s.token = ? AND s.expires_at > ? LIMIT 1`
  ).bind(hashToken(token), new Date().toISOString()).first();
}

async function requireAdmin(request, env) {
  const user = await currentUser(request, env);
  if (!user) return null;
  if (Number(user.is_admin) === 1 || user.role === "admin") return user;
  return null;
}
...
function createSessionCookie(token) {
  ...
  return `${SESSION_COOKIE}=${encodeURIComponent(token)}; HttpOnly; Path=/; Max-Age=${maxAge}; SameSite=Lax; Secure`;
}
