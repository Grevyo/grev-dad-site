(() => {
  ...
  function ensureDialog() {
    ...
    dialog.setAttribute('aria-label', 'Member profile card');
    dialog.innerHTML = `
      <div class="global-profile-card-shell">
        ...
        <div class="global-profile-card-host" data-popover-card-host>
          <div class="global-profile-card-loading">Loading profile…</div>
        </div>
  ...
  function renderProfile(profile) {
    ...
    const host = dialog.querySelector('[data-popover-card-host]');
    if (!window.GrevProfileCard?.create) throw new Error('The shared profile card is unavailable.');
    const card = window.GrevProfileCard.create(profile, { variant: 'popover', className: 'global-profile-card' });
  ...
  async function openProfileCard(profileId) {
    ...
    dialog.querySelector('[data-popover-card-host]').innerHTML = '<div class="global-profile-card-loading">Loading profile…</div>';
    dialog.querySelector('[data-popover-message]').textContent = '';
    ...
    try {
      ...
      dialog.querySelector('[data-popover-card-host]').replaceChildren();
