...
async function handleProfileMe(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(user.id).first(); if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, avatar_url, banner_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const ranks = await loadAccountRanks(request, env); if (!ranks.ok) return json(ranks, 500); const progress = await getUserAccountProgress(db, row.id); const unlockedRanks = getUnlockedRanks(progress.accountLevel, ranks.ranks); const defaultRank = getDefaultRankForLevel(progress.accountLevel, ranks.ranks); const rank = getDisplayedRank(progress.accountLevel, profileRow?.selected_rank_id, ranks.ranks); return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...(profileRow||{}), ...progress, selected_rank_id: profileRow?.selected_rank_id || null, rank, defaultRank, unlockedRanks } }); }
async function handleProfileLookup(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const url = new URL(request.url); const idParam = url.searchParams.get('id'); const usernameParam = (url.searchParams.get('user') || '').trim(); let row = null; if (idParam) { const id = Number(idParam); if (!Number.isInteger(id) || id < 1) return json({ ok: false, error: 'Invalid user id' }, 400); row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(id).first(); } else if (usernameParam) { row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE username = ?').bind(usernameParam).first(); } else { return handleProfileMe(request, env); } if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, avatar_url, banner_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const ranks = await loadAccountRanks(request, env); if (!ranks.ok) return json(ranks, 500); const progress = await getUserAccountProgress(db, row.id); const defaultRank = getDefaultRankForLevel(progress.accountLevel, ranks.ranks); const rank = getDisplayedRank(progress.accountLevel, profileRow?.selected_rank_id, ranks.ranks); const unlockedRanks = user.id === row.id ? getUnlockedRanks(progress.accountLevel, ranks.ranks) : undefined; return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...(profileRow||{}), ...progress, selected_rank_id: profileRow?.selected_rank_id || null, rank, defaultRank, ...(unlockedRanks ? { unlockedRanks } : {}) } }); }
