...
async function ensureCaseDefinition(env, section, now) {
  ...
  let existing = await env.CASES_DB.prepare(`SELECT id, image_url FROM case_definitions WHERE case_name = ? OR slug = ? LIMIT 1`).bind(section.case_name, slug).first();
  if (existing?.id) {
...
export async function importMasterCatalog(env) {
  ...
  for (const section of sections) {
    ...
    for (const item of section.items) {
      ...
      if (existingDrop?.id) {
        await env.CASES_DB.prepare(`UPDATE case_drops SET drop_weight = ? WHERE id = ?`).bind(Number(item.drop_weight || 1), existingDrop.id).run();
      } else {
        await env.CASES_DB.prepare(`INSERT INTO case_drops (case_id, item_id, drop_weight) VALUES (?, ?, ?)`).bind(caseId, itemId, Number(item.drop_weight || 1)).run();
        stats.drops_added += 1;
      }
    ...
    const existingDrops = await env.CASES_DB.prepare(`SELECT id, item_id FROM case_drops WHERE case_id = ?`).bind(caseId).all();
    for (const drop of existingDrops.results || []) {
      if (!desiredItemIds.has(Number(drop.item_id))) {
        await env.CASES_DB.prepare(`DELETE FROM case_drops WHERE id = ?`).bind(drop.id).run();
