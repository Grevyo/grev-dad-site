...
function normalizeProfileWidgetSettingsJson(input) {
  ...
  try { const parsed = JSON.parse(raw); if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return { error: 'profile_page_widget_settings_json must be a JSON object' }; } catch { return { error: 'profile_page_widget_settings_json must be valid JSON' }; }
  return { value: raw };
}
