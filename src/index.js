...
const TILE_MINIMUM_SIZES = { 'profile-snapshot': { minW: 4, minH: 2 }, 'quick-actions': { minW: 2, minH: 2 }, 'profile-completion': { minW: 2, minH: 2 }, 'members-preview': { minW: 4, minH: 2 }, chat: { minW: 4, minH: 4 }, 'coming-later': { minW: 2, minH: 2 }, status: { minW: 2, minH: 2 }, blank: { minW: 1, minH: 1 } };
...
function normalizeProfileInput(body) {
  ...
  return { display_name, profile_title, bio, location, website_url, steam_url, avatar_url, banner_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active };
}
...
function normalizeHomepageTilePayload(tileId, body) { const allowed = Array.isArray(body?.allowed_sizes) ? body.allowed_sizes.map((s) => String(s || '').trim()).filter(Boolean) : []; const invalid = allowed.filter((size) => !HOMEPAGE_TILE_SIZE_OPTIONS.includes(size)); if (!allowed.length) return { error: 'allowed_sizes must contain at least one size' }; if (invalid.length) return { error: `Unsupported size(s): ${invalid.join(', ')}` }; const min=TILE_MINIMUM_SIZES[tileId]||TILE_MINIMUM_SIZES.blank; const tooSmall=allowed.some((size)=>{const m=String(size).match(/^(\d+)x(\d+)$/); return m && (Number(m[1])<min.minW||Number(m[2])<min.minH);}); if(tooSmall) return { error: 'Size is too small for this tile.' }; const defaultSize = String(body?.default_size || '').trim(); if (!allowed.includes(defaultSize)) return { error: 'default_size must be one of allowed_sizes' }; const dm=String(defaultSize).match(/^(\d+)x(\d+)$/); const min=TILE_MINIMUM_SIZES[tileId]||TILE_MINIMUM_SIZES.blank; if(dm && (Number(dm[1])<min.minW||Number(dm[2])<min.minH)) return { error: 'Size is too small for this tile.' }; const isEnabled = Number(body?.is_enabled); if (!(isEnabled === 0 || isEnabled === 1)) return { error: 'is_enabled must be 0 or 1' }; return { tile_id: tileId, label: String(body?.label || '').trim() || tileId, default_size: defaultSize, allowed_sizes: allowed, is_enabled: isEnabled, sort_order: Number(body?.sort_order) || 0 }; }
