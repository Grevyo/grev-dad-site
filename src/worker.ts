...
export default {
  async fetch(request: Request, env: AppEnv): Promise<Response> {
    ...
    if (request.method === 'GET' && (url.pathname === '/hub' || url.pathname === '/members')) {
      ...
      const page = url.pathname === '/members' ? '/members.html' : '/hub.html';
      return session.authenticated ? (env as unknown as DashboardEnv).ASSETS.fetch(new Request(new URL(page, url).toString(), request)) : Response.redirect(new URL('/login', url).toString(), 303);
    }
