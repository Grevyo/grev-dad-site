...
const CS_CARD_TILE_KEYS = ['faceit_overview','faceit_level','faceit_elo','faceit_recent_form','faceit_last_match','faceit_win_rate','faceit_matches_tracked','cs2_overview','cs2_premier_rank','cs2_recent_form','cs2_last_match','cs2_last_map','cs2_last_score','cs2_matches_tracked','cs2_last_synced'];
...
async function handleFaceitConnectionGet(request, env) { const ctx=await getOwnerConnectionUser(request, env); if(ctx.error) return ctx.error; const userId=String(ctx.user.id); const row=await ctx.db.prepare('SELECT * FROM faceit_connections WHERE user_id=?').bind(userId).first(); const stats=await ctx.db.prepare('SELECT * FROM faceit_public_stats WHERE user_id=?').bind(userId).first().catch(()=>null); return json({ ok:true, connection:serializeFaceitConnection(row), publicStats:publicFaceitStatsFromRow(stats) }, 200, { 'Cache-Control':'no-store' }); }
