...
async function getSingleCardPriceCoins(env) {
  const row = await getCachedValue('casesdb:ygo:single-price', 30000, async () => await env.CASES_DB.prepare(`SELECT value FROM ygo_settings WHERE key = 'single_card_price_coins' LIMIT 1`).first());
...
export async function handleYgoRequest(request, env, deps) {
  ...
  if (pathname === '/api/ygo/packs' && request.method === 'GET') {
    const payload = await getCachedValue('casesdb:ygo:packs', 60000, async () => {
      ...
      for (const pack of packs.results || []) {
        ...
        const packCards = await env.CASES_DB.prepare(`SELECT estimated_price_coins, drop_weight FROM ygo_cards WHERE pack_slug = ?`).bind(pack.slug).all();
  ...
  if (pathname === '/api/ygo/admin/settings' && request.method === 'POST') {
    ...
    await env.CASES_DB.prepare(`INSERT INTO ygo_settings (key, value) VALUES ('single_card_price_coins', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value`).bind(String(price)).run();
  ...
  if (pathname === '/api/ygo/admin/packs' && request.method === 'GET') {
    ...
    const packs = await env.CASES_DB.prepare(`SELECT * FROM ygo_pack_definitions ORDER BY set_name ASC`).all();
    const payload = [];
    for (const pack of packs.results || []) {
      const cards = await env.CASES_DB.prepare(`SELECT id, card_name, rarity_code, estimated_price_coins, drop_weight FROM ygo_cards WHERE pack_slug = ? ORDER BY estimated_price_coins DESC, card_name ASC`).bind(pack.slug).all();
  ...
  if (pathname === '/api/ygo/admin/roi' && request.method === 'POST') {
    ...
    const cards = await env.CASES_DB.prepare(`SELECT estimated_price_coins, drop_weight FROM ygo_cards WHERE pack_slug = ?`).bind(packSlug).all();
    const expectedValue = calculatePackExpectedValue(cards.results || []);
  ...
  if (pathname === '/api/ygo/profile/public' && request.method === 'GET') {
    ...
    const showcaseRows = await env.CASES_DB.prepare(`
  ...
  if (pathname === '/api/ygo/inventory' && request.method === 'GET') {
    ...
    const showcaseRows = await env.CASES_DB.prepare(`SELECT slot, inventory_id FROM ygo_showcase WHERE user_id = ? ORDER BY slot ASC`).bind(session.id).all();
    return json({
      success: true,
  ...
  if (pathname === '/api/ygo/showcase' && request.method === 'POST') {
    ...
    if (inventoryId === 0) {
      await env.CASES_DB.prepare(`DELETE FROM ygo_showcase WHERE user_id = ? AND slot = ?`).bind(session.id, slot).run();
      return json({ success: true, message: 'Showcase slot cleared' }, 200, request);
    }

    const item = await env.CASES_DB.prepare(`SELECT id FROM ygo_inventory WHERE id = ? AND user_id = ? AND sold_at IS NULL LIMIT 1`).bind(inventoryId, session.id).first();
    if (!item) {
  ...
  if (pathname === '/api/ygo/single/open' && request.method === 'POST') {
    const priceRow = await env.CASES_DB.prepare(`SELECT value FROM ygo_settings WHERE key = 'single_card_price_coins' LIMIT 1`).first();
    ...
    const cards = await env.CASES_DB.prepare(`SELECT * FROM ygo_cards`).all();
    ...
    await env.CASES_DB.batch([
      env.CASES_DB.prepare(`UPDATE case_profiles SET balance = balance - ?, updated_at = ? WHERE user_id = ?`).bind(price, now, session.id),
      env.CASES_DB.prepare(`INSERT INTO ygo_inventory (user_id, pack_slug, card_id, card_name, rarity_code, foil_label, sell_back_coins, estimated_price_coins, image_url, acquired_at, source_pack_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`).bind(session.id, pull.pack_slug, pull.id, pull.card_name, pull.rarity_code, formatFoilLabel(pull.rarity_code), sellBack, Number(pull.estimated_price_coins || 0), pull.image_url || '', now),
      env.CASES_DB.prepare(`UPDATE ygo_player_stats SET cards_owned = cards_owned + 1, updated_at = ? WHERE user_id = ?`).bind(now, session.id)
    ]);
    const profile = await buildProfilePayload(env, session);
