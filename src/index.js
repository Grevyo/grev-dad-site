...
async function handleProfileMe(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); await ensureStarterUnlocks(db, user.id); const row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(user.id).first(); if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, steam_url, avatar_url, banner_url, profile_background_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, status_message, profile_quote, favourite_game, profile_visibility, show_member_card, show_profile_showcase, show_profile_xp, show_profile_user_id, show_joined_date, show_header_avatar, show_header_display_name, show_header_username, show_header_user_id, show_header_level, show_header_rank, show_header_xp_bar, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const safeProfile = { display_name: row.username, bio: '', profile_title: '', avatar_url: '', banner_url: '', profile_background_url: '', profile_layout: 'standard', show_level: 1, show_rank: 1, show_badges: 1, show_last_active: 0, card_background_url: '', card_background_colour: '', card_accent_colour: '', card_text_colour: '', card_border_colour: '', card_layout: 'standard', card_show_avatar: 1, card_show_display_name: 1, card_show_username: 1, card_show_user_id: 0, card_show_role: 1, card_show_level: 1, card_show_rank: 1, card_show_xp: 1, card_show_status: 0, card_show_steam: 0, ...(profileRow || {}) }; const progress = await getUserAccountProgress(db, row.id); const ranks = await loadAccountRanks(request, env); const rankLoadError = !ranks.ok; const rankList = ranks.ok ? ranks.ranks : []; const unlockedRanks = getUnlockedRanks(progress.accountLevel, rankList); const defaultRank = getDefaultRankForLevel(progress.accountLevel, rankList); const rank = getDisplayedRank(progress.accountLevel, safeProfile?.selected_rank_id, rankList); const showcase = await getPublicShowcaseSlots(db, row.id); return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...safeProfile, ...progress, selected_rank_id: safeProfile?.selected_rank_id || null, rank, defaultRank, unlockedRanks, showcase }, ...(rankLoadError ? { rankLoadError: ranks.error } : {}) }); }
async function handleProfileLookup(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const url = new URL(request.url); const idParam = url.searchParams.get('id'); const usernameParam = (url.searchParams.get('user') || '').trim(); let row = null; if (idParam) { const id = Number(idParam); if (!Number.isInteger(id) || id < 1) return json({ ok: false, error: 'Invalid user id' }, 400); row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE id = ?').bind(id).first(); } else if (usernameParam) { row = await db.prepare('SELECT id, username, role, is_admin, created_at, status FROM users WHERE username = ?').bind(usernameParam).first(); } else { return handleProfileMe(request, env); } if (!row) return json({ ok: false, error: 'User not found' }, 404); const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, steam_url, avatar_url, banner_url, profile_background_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, status_message, profile_quote, favourite_game, profile_visibility, show_member_card, show_profile_showcase, show_profile_xp, show_profile_user_id, show_joined_date, show_header_avatar, show_header_display_name, show_header_username, show_header_user_id, show_header_level, show_header_rank, show_header_xp_bar, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first(); const safeProfile = { display_name: row.username, bio: '', profile_title: '', avatar_url: '', banner_url: '', profile_background_url: '', profile_layout: 'standard', show_level: 1, show_rank: 1, show_badges: 1, show_last_active: 0, card_background_url: '', card_background_colour: '', card_accent_colour: '', card_text_colour: '', card_border_colour: '', card_layout: 'standard', card_show_avatar: 1, card_show_display_name: 1, card_show_username: 1, card_show_user_id: 0, card_show_role: 1, card_show_level: 1, card_show_rank: 1, card_show_xp: 1, card_show_status: 0, card_show_steam: 0, ...(profileRow || {}) }; const progress = await getUserAccountProgress(db, row.id); const ranks = await loadAccountRanks(request, env); const rankLoadError = !ranks.ok; const rankList = ranks.ok ? ranks.ranks : []; const defaultRank = getDefaultRankForLevel(progress.accountLevel, rankList); const rank = getDisplayedRank(progress.accountLevel, safeProfile?.selected_rank_id, rankList); const unlockedRanks = user.id === row.id ? getUnlockedRanks(progress.accountLevel, rankList) : undefined; const showcase = await getPublicShowcaseSlots(db, row.id); return json({ ok: true, profile: { ...serializeUser(row), created_at: row.created_at || null, ...safeProfile, ...progress, selected_rank_id: safeProfile?.selected_rank_id || null, rank, defaultRank, showcase, ...(unlockedRanks ? { unlockedRanks } : {}) }, ...(rankLoadError ? { rankLoadError: ranks.error } : {}) }); }
async function handleProfileMeUpdate(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const body = await readJsonBody(request); const profile = normalizeProfileInput(body); if (profile.error) return json({ ok: false, error: profile.error }, 400); const db = getDatabase(env); await ensureSchema(db); await db.prepare("INSERT INTO user_profiles (user_id, display_name, profile_title, bio, location, website_url, steam_url, avatar_url, banner_url, profile_background_url, status_message, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now')) ON CONFLICT(user_id) DO UPDATE SET display_name=excluded.display_name, profile_title=excluded.profile_title, bio=excluded.bio, location=excluded.location, website_url=excluded.website_url, steam_url=excluded.steam_url, avatar_url=excluded.avatar_url, banner_url=excluded.banner_url, profile_background_url=excluded.profile_background_url, status_message=excluded.status_message, favourite_colour=excluded.favourite_colour, profile_accent_colour=excluded.profile_accent_colour, profile_background_colour=excluded.profile_background_colour, profile_layout=excluded.profile_layout, show_level=excluded.show_level, show_rank=excluded.show_rank, show_badges=excluded.show_badges, show_last_active=excluded.show_last_active, updated_at=datetime('now')").bind(user.id, profile.display_name, profile.profile_title, profile.bio, profile.location, profile.website_url, profile.steam_url, profile.avatar_url, profile.banner_url, profile.profile_background_url, normalizeNullableString(body?.status_message, 140), profile.favourite_colour, profile.profile_accent_colour, profile.profile_background_colour, profile.profile_layout, profile.show_level, profile.show_rank, profile.show_badges, profile.show_last_active).run(); return handleProfileMe(request, env); } catch (error) { return json({ ok: false, error: friendlyError(error) }, 500); } }
async function handleProfileCardMeUpdate(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const body = await readJsonBody(request); const card_layout = ['compact', 'standard', 'showcase'].includes(String(body?.card_layout || 'standard')) ? String(body?.card_layout || 'standard') : null; if (!card_layout) return json({ ok: false, error: 'Invalid card_layout' }, 400); const card_background_url = normalizeImageUrl(body?.card_background_url); if (card_background_url === null) return json({ ok: false, error: 'Invalid card image URL' }, 400); const db = getDatabase(env); await ensureSchema(db); await db.prepare("INSERT INTO user_profiles (user_id, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now')) ON CONFLICT(user_id) DO UPDATE SET card_background_url=excluded.card_background_url, card_background_colour=excluded.card_background_colour, card_accent_colour=excluded.card_accent_colour, card_text_colour=excluded.card_text_colour, card_border_colour=excluded.card_border_colour, card_layout=excluded.card_layout, card_show_avatar=excluded.card_show_avatar, card_show_display_name=excluded.card_show_display_name, card_show_username=excluded.card_show_username, card_show_user_id=excluded.card_show_user_id, card_show_role=excluded.card_show_role, card_show_level=excluded.card_show_level, card_show_rank=excluded.card_show_rank, card_show_xp=excluded.card_show_xp, card_show_status=excluded.card_show_status, card_show_steam=excluded.card_show_steam, updated_at=datetime('now')").bind(user.id, card_background_url || '', normalizeHexColour(body?.card_background_colour), normalizeHexColour(body?.card_accent_colour), normalizeHexColour(body?.card_text_colour), normalizeHexColour(body?.card_border_colour), card_layout, normalizeBool01(body?.card_show_avatar, 1), normalizeBool01(body?.card_show_display_name, 1), normalizeBool01(body?.card_show_username, 1), normalizeBool01(body?.card_show_user_id, 0), normalizeBool01(body?.card_show_role, 1), normalizeBool01(body?.card_show_level, 1), normalizeBool01(body?.card_show_rank, 1), normalizeBool01(body?.card_show_xp, 1), normalizeBool01(body?.card_show_status, 0), normalizeBool01(body?.card_show_steam, 0)).run(); return handleProfileMe(request, env); } catch (error) { return json({ ok: false, error: friendlyError(error) }, 500); } }
...
function normalizeProfileInput(body) {
  ...
  return { display_name, profile_title, bio, location, website_url, steam_url, avatar_url, banner_url, profile_background_url, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active };
}
