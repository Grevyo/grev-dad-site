...
const CARD_TILE_KEYS = new Set(['name','username','role','level','rank','xp','status','steam','leetify','leetify_rank','leetify_rating']);
...
function normalizeProfileInput(body) {
  ...
  const profile_footer_url = normalizeImageUrl(body?.profile_footer_url);
  const profile_footer_display_size = ['compact','wide','tall','full'].includes(String(body?.profile_footer_display_size||'wide')) ? String(body?.profile_footer_display_size||'wide') : null;
  ...
  const profile_page_background_url = normalizeImageUrl(body?.profile_page_background_url);
  if (profile_page_background_url === null) return { error: 'Invalid profile_page_background_url' };
  const profile_page_background_colour = clean(body?.profile_page_background_colour);
  ...
  const profile_page_background_size = ['cover','contain','repeat','stretch','center'].includes(String(body?.profile_page_background_size||'cover')) ? String(body?.profile_page_background_size||'cover') : null;
  if (!profile_page_background_size) return { error: 'Invalid profile_page_background_size' };
  const profile_page_overlay_strength = Number(body?.profile_page_overlay_strength ?? 20);
...
async function ensureUserProfileColumns(db) {
  ...
  const required = ['steam_url','leetify_url','banner_display_size','profile_background_url','profile_background_size','card_background_url','card_background_colour','card_accent_colour','card_border_colour','card_layout','card_show_avatar','card_show_display_name','card_show_username','card_show_user_id','card_show_role','card_show_level','card_show_rank','card_show_xp','card_show_status','card_show_steam','card_show_leetify','card_show_leetify_rank','card_show_leetify_rating','card_grid_columns','card_tile_settings_json'];
...
async function getUserProfileBundle(request, env, userId, { includeUnlockedRanks = false } = {}) {
  ...
  if (!row) return null;
  const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, steam_url, leetify_url, avatar_url, banner_url, banner_display_size, profile_background_url, profile_background_size, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, status_message, profile_quote, favourite_game, profile_visibility, show_member_card, show_profile_showcase, show_profile_xp, show_profile_user_id, show_joined_date, show_header_avatar, show_header_display_name, show_header_username, show_header_user_id, show_header_level, show_header_rank, show_header_xp_bar, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_body_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, card_show_leetify, card_show_leetify_rank, card_show_leetify_rating, card_grid_columns, card_tile_settings_json, profile_page_background_url, profile_page_background_colour, profile_page_background_size, profile_page_overlay_strength, profile_page_tile_layout_json, profile_page_widget_settings_json, profile_footer_url, profile_footer_display_size, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first();
...
async function handleBootstrapDashboard(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok:false, error:'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const ranksPromise = loadAccountRanks(request, env); const batchResults = await db.batch([db.prepare('SELECT display_name, profile_title, bio, avatar_url, banner_url, selected_rank_id, status_message, show_level, show_rank, card_tile_settings_json FROM user_profiles WHERE user_id = ?').bind(user.id), db.prepare('SELECT tile_id, label, default_size, allowed_sizes_json, is_enabled, sort_order FROM homepage_tile_config ORDER BY sort_order ASC, tile_id ASC'), db.prepare('SELECT account_xp, account_level FROM users WHERE id = ?').bind(user.id)]);
  const [profileRes, tileRes, progressRes] = batchResults;
  const ranksData = await ranksPromise;
  const rankList = ranksData.ok ? ranksData.ranks : [];
  const profileRow = profileRes?.results?.[0] || {};
  const progress = { accountXp: Number(progressRes?.results?.[0]?.account_xp || 0), accountLevel: Number(progressRes?.results?.[0]?.account_level || 1), accountXpCurrent: Number(progressRes?.results?.[0]?.account_xp || 0), accountXpRequired: getRequiredXpForLevel(Number(progressRes?.results?.[0]?.account_level || 1)) };
  const rank = getDisplayedRank(progress.accountLevel, profileRow?.selected_rank_id, rankList);
  const profile = { ...serializeUser(user), ...profileRow, ...progress, rank, cardTileSettings: normalizeCardTileSettings(profileRow?.card_tile_settings_json) };
  const homepageTileConfig = (tileRes?.results || []).map((row) => ({ tile_id: row.tile_id, label: row.label, default_size: row.default_size, allowed_sizes: parseAllowedSizes(row.allowed_sizes_json), is_enabled: Number(row.is_enabled) === 1 ? 1 : 0, sort_order: Number(row.sort_order) || 0 }));
  return json({ ok:true, user: serializeUser(user), profile, homepageTileConfig, ranks: rankList, serverTime: new Date().toISOString() }, 200, { 'Cache-Control':'private, max-age=15' }); } catch (error) { return json({ ok:false, error:friendlyError(error) }, 500); } }
