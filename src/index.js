...
async function fetchNextCs2ShareCode({apiKey,steamId64,authCode,knownCode}){ const params=new URLSearchParams({ key:apiKey, steamid:steamId64, steamidkey:authCode, knowncode:knownCode }); const url=`https://api.steampowered.com/ICSGOPlayers_730/GetNextMatchSharingCode/v1/?${params.toString()}`; let resp; try { resp=await fetch(url,{headers:{'Accept':'application/json,text/plain,*/*'}}); } catch { return { ok:false, error:'Valve match-history request failed.' }; } const text=await resp.text().catch(()=>''); let payload=null; if(/^\s*[\[{]/.test(text)){ try{ payload=JSON.parse(text); }catch{} } if(!resp.ok) return { ok:false, error:resp.status===403?'Valve denied the match-history request. Check the saved CS2 Authentication Code and latest match token.':'Valve match-history request failed.' }; if(/access is denied|forbidden|unauthorized|verify your key/i.test(text)) return { ok:false, error:'Valve denied the match-history request. Check the server Steam API key and saved CS2 details.' }; const nextCode=textOrNull(payload?.result?.nextcode||payload?.response?.nextcode||payload?.nextcode||payload?.result?.next_code); return { ok:true, nextCode, payload }; }
async function discoverCs2ShareCodes({apiKey,steamId64,authCode,latestShareCode}){ const discovered=[]; const seen=new Set([String(latestShareCode||'')]); let knownCode=latestShareCode; for(let i=0;i<10;i++){ const next=await fetchNextCs2ShareCode({apiKey,steamId64,authCode,knownCode}); if(!next.ok) return { ok:false, error:next.error, discovered }; if(!next.nextCode || seen.has(next.nextCode)) return { ok:true, discovered }; discovered.push(next.nextCode); seen.add(next.nextCode); knownCode=next.nextCode; } return { ok:true, discovered, truncated:true }; }
