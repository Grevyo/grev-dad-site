(function () {
  ...
  function initRangeSliders(root = document) {
    root.querySelectorAll('input[type="range"]').forEach((input) => {
      ...
      if (!input.dataset.rangeProgressBound) {
        input.addEventListener('input', () => updateRangeProgress(input));
        input.addEventListener('change', () => updateRangeProgress(input));
        input.dataset.rangeProgressBound = '1';
      }
    });
  }
  window.GREVTheme = { applyTheme: setTheme, toggleTheme, syncButton };
  window.GREVUi = { updateRangeProgress, initRangeSliders };
  applyTheme(getStoredTheme());
  document.addEventListener('DOMContentLoaded', () => { syncButton(document.documentElement.dataset.theme || DARK); const b=document.getElementById('theme-toggle'); if (b) b.addEventListener('click', toggleTheme); initRangeSliders(document); });
})();
