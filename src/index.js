...
const PROFILE_WIDGET_KEYS = new Set(['profile-card','bio','status','steam','leetify','showcase','links','progress']);
...
function base64ToBytes(text) { const binary = atob(String(text || '')); return Uint8Array.from(binary, (c) => c.charCodeAt(0)); }
async function deriveSecretKey(secret) { const material = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(String(secret || ''))); return crypto.subtle.importKey('raw', material, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']); }
async function encryptSecret(plain, env) { const secret = env?.APP_SECRET_KEY || env?.PROFILE_SECRET_KEY; if (!secret) throw new Error('Steam API key storage is not configured. Add APP_SECRET_KEY as a Worker secret first.'); const iv = crypto.getRandomValues(new Uint8Array(12)); const key = await deriveSecretKey(secret); const cipher = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode(String(plain || '')))); return JSON.stringify({ v: 1, alg: 'AES-GCM', iv: bytesToBase64(iv), ct: bytesToBase64(cipher) }); }
async function decryptSecret(encrypted, env) { const secret = env?.APP_SECRET_KEY || env?.PROFILE_SECRET_KEY; if (!secret || !encrypted) return ''; try { const payload = JSON.parse(String(encrypted)); if (payload?.v !== 1 || !payload?.iv || !payload?.ct) return ''; const key = await deriveSecretKey(secret); const plain = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: base64ToBytes(payload.iv) }, key, base64ToBytes(payload.ct)); return new TextDecoder().decode(plain); } catch { return ''; } }
...
async function ensureUserProfileColumns(db) {
  ...
  const required = ['steam_url','leetify_url','refrag_url','steam_api_key_encrypted','banner_display_size','profile_background_url','profile_background_size','card_background_url','card_background_colour','card_accent_colour','card_border_colour','card_layout','card_show_avatar','card_show_display_name','card_show_username','card_show_user_id','card_show_role','card_show_level','card_show_rank','card_show_xp','card_show_status','card_show_steam','card_show_refrag','card_show_leetify','card_show_leetify_rank','card_show_leetify_rating','card_grid_columns','card_tile_settings_json'];
...
async function createSchemaTables(db) { await db.prepare("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'admin', is_admin INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')))" ).run(); try { await db.prepare("ALTER TABLE users ADD COLUMN status TEXT NOT NULL DEFAULT 'active'").run(); } catch {}
  ...
  try { await db.prepare("ALTER TABLE user_profiles ADD COLUMN steam_api_key_encrypted TEXT DEFAULT ''").run(); } catch {}
...
async function getUserProfileBundle(request, env, userId, { includeUnlockedRanks = false } = {}) {
  ...
  safeProfile.has_steam_api_key = includeUnlockedRanks ? Boolean(safeProfile.steam_api_key_encrypted) : undefined; delete safeProfile.steam_api_key_encrypted; if (safeProfile.has_steam_api_key === undefined) delete safeProfile.has_steam_api_key;
  ...
  return { profile: { ...serializeUser(row), created_at: row.created_at || null, ...safeProfile, ...progress, selected_rank_id: safeProfile?.selected_rank_id || null, rank, defaultRank, showcase, ...(unlockedRanks ? { unlockedRanks } : {}) }, ranks };
}
...
async function handleSteamProfile(request, env) {
  ...
  const userId = requestedId ? Number(requestedId) : Number(user.id);
  if(!Number.isInteger(userId)||userId<1) return json({ ok:false, error:'Invalid user id' },400);
  const row=await db.prepare('SELECT steam_url, steam_api_key_encrypted FROM user_profiles WHERE user_id = ?').bind(userId).first();
  ...
  try {
    ...
    const apiKey=env.STEAM_API_KEY || await decryptSecret(row?.steam_api_key_encrypted, env);
    if (apiKey) {
      try {
