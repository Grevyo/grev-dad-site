...
const CARD_TILE_KEYS = new Set(['name','username','role','level','rank','xp','status','steam','refrag','leetify','leetify_rank','leetify_rating']);
const PROFILE_WIDGET_KEYS = new Set(['profile-card','bio','status','steam','leetify','showcase','links','progress','achievements']);
...
export default { async fetch(request, env) { const url = new URL(request.url);
  ...
  if (url.pathname === '/api/bootstrap/dashboard' && request.method === 'GET') return handleBootstrapDashboard(request, env);
  if (url.pathname === '/api/bootstrap/profile' && request.method === 'GET') return handleBootstrapProfile(request, env);
...
async function handleMe(request, env) { const u = await getCurrentUserFast(request, env); if (!u) return json({ ok: false, user: null, error: 'Not logged in' }, 401); return json({ ok: true, user: { id: u.id, username: u.username, role: u.role, roleLabel: getRoleLabel(u.role), roleLevel: getRoleLevel(u.role), is_admin: u.is_admin } }, 200, {'Cache-Control':'private, max-age=15'}); }

const PUBLIC_PROFILE_CARD_COLUMNS = 'p.display_name, p.profile_title, p.avatar_url, p.status_message, p.steam_url, p.leetify_url, p.refrag_url, p.card_background_url, p.card_background_colour, p.card_accent_colour, p.card_border_colour, p.card_text_colour, p.card_body_text_colour, p.card_layout, p.card_grid_columns, p.card_show_avatar, p.card_show_display_name, p.card_show_username, p.card_show_user_id, p.card_show_role, p.card_show_level, p.card_show_rank, p.card_show_xp, p.card_show_status, p.card_show_steam, p.card_show_leetify, p.card_show_leetify_rank, p.card_show_leetify_rating, p.card_show_refrag, p.card_tile_settings_json';
function publicProfileCardFields(row = {}) { const out = { display_name: row.display_name || null, profile_title: row.profile_title || null, avatar_url: row.avatar_url || null, status_message: row.status_message || '', steam_url: row.steam_url || '', leetify_url: row.leetify_url || '', refrag_url: row.refrag_url || '', card_background_url: row.card_background_url || '', card_background_colour: row.card_background_colour || '', card_accent_colour: row.card_accent_colour || '', card_border_colour: row.card_border_colour || '', card_text_colour: row.card_text_colour || '', card_body_text_colour: row.card_body_text_colour || '', card_layout: row.card_layout || 'standard', card_grid_columns: Number(row.card_grid_columns) || 4, card_show_avatar: row.card_show_avatar == null ? 1 : Number(row.card_show_avatar), card_show_display_name: row.card_show_display_name == null ? 1 : Number(row.card_show_display_name), card_show_username: row.card_show_username == null ? 1 : Number(row.card_show_username), card_show_user_id: row.card_show_user_id == null ? 0 : Number(row.card_show_user_id), card_show_role: row.card_show_role == null ? 1 : Number(row.card_show_role), card_show_level: row.card_show_level == null ? 1 : Number(row.card_show_level), card_show_rank: row.card_show_rank == null ? 1 : Number(row.card_show_rank), card_show_xp: row.card_show_xp == null ? 1 : Number(row.card_show_xp), card_show_status: row.card_show_status == null ? 0 : Number(row.card_show_status), card_show_steam: row.card_show_steam == null ? 0 : Number(row.card_show_steam), card_show_leetify: row.card_show_leetify == null ? 0 : Number(row.card_show_leetify), card_show_leetify_rank: row.card_show_leetify_rank == null ? 0 : Number(row.card_show_leetify_rank), card_show_leetify_rating: row.card_show_leetify_rating == null ? 0 : Number(row.card_show_leetify_rating), card_show_refrag: row.card_show_refrag == null ? 0 : Number(row.card_show_refrag), card_tile_settings_json: row.card_tile_settings_json || '' }; out.cardTileSettings = normalizeCardTileSettings(out.card_tile_settings_json); return out; }
...
async function handleRanks(request, env) { const user = await getCurrentUser(request, env); if (!user) return json({ ok: false, error: 'Not logged in' }, 401); const ranks = await loadAccountRanks(request, env); if (!ranks.ok) return json(ranks, 500); return json({ ok: true, ranks: ranks.ranks }, 200, {'Cache-Control':'private, max-age=60'}); }
...
function normalizeProfileInput(body, existingProfile = {}) {
  ...
  const refrag_url = nullable(body?.refrag_url);
  ...
  if (refrag_url && refrag_url.length > 300) return { error: 'refrag_url max length is 300' };
  ...
  if (refrag_url && !isSafeHttpUrl(refrag_url)) return { error: 'refrag_url must be a safe http/https URL' };
...
async function ensureUserProfileColumns(db) {
  ...
  const required = ['steam_url','leetify_url','refrag_url','banner_display_size','profile_background_url','profile_background_size','card_background_url','card_background_colour','card_accent_colour','card_border_colour','card_layout','card_show_avatar','card_show_display_name','card_show_username','card_show_user_id','card_show_role','card_show_level','card_show_rank','card_show_xp','card_show_status','card_show_steam','card_show_refrag','card_show_leetify','card_show_leetify_rank','card_show_leetify_rating','card_grid_columns','card_tile_settings_json'];
  const missing = required.filter((name) => !names.has(name));
  if (missing.length) console.error('[schema] user_profiles missing columns', missing);
}
...
async function getUserProfileBundle(request, env, userId, { includeUnlockedRanks = false } = {}) {
  ...
  if (!row) return null;
  const profileRow = await db.prepare('SELECT display_name, profile_title, bio, location, website_url, steam_url, leetify_url, refrag_url, avatar_url, banner_url, banner_display_size, profile_background_url, profile_background_size, favourite_colour, profile_accent_colour, profile_background_colour, profile_layout, show_level, show_rank, show_badges, show_last_active, status_message, profile_quote, favourite_game, profile_visibility, show_member_card, show_profile_showcase, show_profile_xp, show_profile_user_id, show_joined_date, show_header_avatar, show_header_display_name, show_header_username, show_header_user_id, show_header_level, show_header_rank, show_header_xp_bar, card_background_url, card_background_colour, card_accent_colour, card_text_colour, card_body_text_colour, card_border_colour, card_layout, card_show_avatar, card_show_display_name, card_show_username, card_show_user_id, card_show_role, card_show_level, card_show_rank, card_show_xp, card_show_status, card_show_steam, card_show_refrag, card_show_leetify, card_show_leetify_rank, card_show_leetify_rating, card_grid_columns, card_tile_settings_json, profile_page_background_url, profile_page_background_colour, profile_page_background_size, profile_page_overlay_strength, profile_page_tile_layout_json, profile_page_widget_settings_json, profile_footer_url, profile_footer_display_size, selected_rank_id, updated_at FROM user_profiles WHERE user_id = ?').bind(row.id).first();
  ...
  safeProfile.cardTileSettings = normalizeCardTileSettings(safeProfile.card_tile_settings_json);
  ...
  const showcase = await getPublicShowcaseSlots(db, row.id);
  const achievements = await getPublicAchievementSummary(db, row.id);
...
async function handleBootstrapDashboard(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok:false, error:'Not logged in' }, 401); const db = getDatabase(env); await ensureSchema(db); const bundlePromise = getUserProfileBundle(request, env, Number(user.id), { includeUnlockedRanks: true }); const tileRes = await db.prepare('SELECT tile_id, label, default_size, allowed_sizes_json, is_enabled, sort_order FROM homepage_tile_config ORDER BY sort_order ASC, tile_id ASC').all(); const bundle = await bundlePromise; if (!bundle) return json({ ok:false, error:'User not found' }, 404); const rankList = bundle.ranks?.ok ? bundle.ranks.ranks : []; const homepageTileConfig = (tileRes?.results || []).map((row) => ({ tile_id: row.tile_id, label: row.label, default_size: row.default_size, allowed_sizes: parseAllowedSizes(row.allowed_sizes_json), is_enabled: Number(row.is_enabled) === 1 ? 1 : 0, sort_order: Number(row.sort_order) || 0 })); return json({ ok:true, user: serializeUser(user), profile: bundle.profile, homepageTileConfig, ranks: rankList, serverTime: new Date().toISOString() }, 200, { 'Cache-Control':'private, max-age=15' }); } catch (error) { return json({ ok:false, error:friendlyError(error) }, 500); } }
...
async function handleLeetifyProfile(request, env) {
  ...
  if (!candidates.length) return json(writeLeetifyCache(cacheKey, unavailable('Leetify profile URL could not be parsed')));
