...
async function handleBootstrapDashboard(request, env) { try { const user = await getCurrentUser(request, env); if (!user) return json({ ok:false, error:'Not logged in' }, 401); const db = getDatabase(env); await ensureSchemaOnce(db); const bundlePromise = getUserProfileBundle(request, env, Number(user.id), { includeUnlockedRanks: true }); const tileRes = await db.prepare('SELECT tile_id, label, default_size, allowed_sizes_json, is_enabled, sort_order FROM homepage_tile_config ORDER BY sort_order ASC, tile_id ASC').all(); const bundle = await bundlePromise; if (!bundle) return json({ ok:false, error:'User not found' }, 404); const rankList = bundle.ranks?.ok ? bundle.ranks.ranks : []; const homepageTileConfig = (tileRes?.results || []).map((row) => ({ tile_id: row.tile_id, label: row.label, default_size: row.default_size, allowed_sizes: parseAllowedSizes(row.allowed_sizes_json), is_enabled: Number(row.is_enabled) === 1 ? 1 : 0, sort_order: Number(row.sort_order) || 0 })); return json({ ok:true, user: serializeUser(user), profile: bundle.profile, homepageTileConfig, ranks: rankList, serverTime: new Date().toISOString() }, 200, { 'Cache-Control':'private, max-age=15' }); } catch (error) { return json({ ok:false, error:friendlyError(error) }, 500); } }
